...
if (re.test(userAgent)) {
  axios.interceptors.request.use(function (config) {
    const token =
      "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VybmFtZSI6Inpib3QiLCJlbWFpbCI6ImJvdEBib3QuYm90IiwibmFtZSI6ImJvdCBhY2Nlc3MiLCJpYXQiOjE2Mjk0Nzk2OTl9.L2oNitBKRLt-vZkkSpHV1vVPZYpg3CA4pT7xYv5xlq4";
...
export default createStore({
  ...
  mutations: {
    ...
    removeWatchListPost(state, id) {
      state.watchListMoviesList = state.watchListMoviesList.filter(
    ...
    fetchHomePosts(state, payload) {
      ...
      state.homePosts = [
        ...new Map(state.homePosts.map((item) => [item["id"], item])).values(),
      ];
    },
    ...
    fetchPostComments(state, payload) {
      ...
      payload.docs.forEach((comment) => {
        ...
        state.postComments = [
          ...new Map(
            state.postComments.map((item) => [item["_id"], item])
          ).values(),
        ];
      });
    },
  ...
  actions: {
    ...
    async getMoviesList({ commit, state, dispatch }) {
      ...
      if (!state.isWatchListLoaded) {
        ...
        for (id in state.watchListMoviesIDs) {
          const options = {
            method: "GET",
            url: `https://www.omdbapi.com/?i=${state.watchListMoviesIDs[id]}&apikey=1f97a893`,
          };
          await axios
            .request(options)
            .then(function (response) {
    ...
    async getSearchList({ state, commit, dispatch }, search) {
      ...
      const options = {
        method: "GET",
        url: `${state.baseURl}/imdb/search?searchTerm=${search}`,
      };
