...
export default {
  ...
  methods: {
    getSearch() {
      let exceptionNames = {
        'moneyheist': 'la casa de papel'
      }
      if(this.searchMov.replace(/\s+/g, '').toLowerCase() in exceptionNames){
        this.searchMov = exceptionNames[this.searchMov.replace(/\s+/g, '').toLowerCase()]
      }
    ...
    addItem(id) {
      ...
      this.toggleWatchListPost(id).then(()=>{
        this.active2 = false;
        this.active = false
